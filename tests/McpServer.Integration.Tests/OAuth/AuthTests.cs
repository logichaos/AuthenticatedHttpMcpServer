...
public class AuthTests : OAuthTestBase
{
    ...
    public async Task CanAuthenticate_WithTokenRefresh()
    {
        ...
        await client.ListToolsAsync(cancellationToken: TestContext.Current.Execution.CancellationToken);

        await Assert.That(TestOAuthServer.HasRefreshedToken).IsTrue();
    }
    ...
    // {
    //     ModelContextProtocol.TestOAuthServer.ExpectResource = false;
    ...
    //     var requests = ModelContextProtocol.TestOAuthServer.MetadataRequests.ToArray();
    ...
    //     ModelContextProtocol.TestOAuthServer.DisabledMetadataPaths.Add($"/.well-known/oauth-authorization-server{issuerPath}");
    //     ModelContextProtocol.TestOAuthServer.DisabledMetadataPaths.Add($"/.well-known/openid-configuration{issuerPath}");
    ...
    //         ],
    //         ModelContextProtocol.TestOAuthServer.MetadataRequests);
    // }
    ...
    //     using var metadataResponse = await System.Net.Http.HttpClient.GetAsync(
    //         "/.well-known/oauth-protected-resource",
    ...
    //     ModelContextProtocol.TestOAuthServer.ValidResources = [resourceWithTrailingSlash, "http://localhost:5000/mcp"];
    ...
    //     var httpClient = System.Net.Http.HttpClient;
    ...
    // {
    //     ModelContextProtocol.TestOAuthServer.IncludeOfflineAccessInMetadata = true;
    //     await using var app = await StartMcpServerAsync();
    ...
    //     Assert.Equal("mcp:tools", ModelContextProtocol.TestOAuthServer.LastRegistrationScope);
    // }
}
