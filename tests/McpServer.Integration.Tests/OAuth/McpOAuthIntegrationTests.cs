...
/// OAuthWebApplicationFactory (in-memory test server with OAuth enabled).
///
/// For full end-to-end OAuth flow tests (authorization redirect, PKCE,
/// token refresh, scope selection), see AuthTests.cs which uses the
/// in-memory TestOAuthServer.
/// </summary>
public class McpOAuthIntegrationTests
{
    ...
    public async Task ProtectedResourceMetadata_ContainsAuthorizationServer()
    {
        ...
        await Assert.That(body).Contains("https://localhost:7029");
    }
